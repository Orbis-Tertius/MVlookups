...
%\frontmatter
\title{%
Multivariate multi-column lookups based on
...
Based on this simple fact, we construct an interactive oracle proof for multi-column lookups over the boolean hypercube, which makes use of a single multiplicity function instead of working with a rearranged union of table and witness.
...
While the number of field operations is comparable to the Hyperplonk+ lookup (extended to multiple columns), the provided by our prover are much less expensive. 
For example, for columns of length $2^{12}$, paper-pencil operation counts indicate that the logarithmic derivative lookup is $1.5$ -- $2.5$ times faster, depending on the number of columns.
\end{abstract}
...
\subsection{Lagrange interactive oracle proofs (TODO)}
...
In  the first one, explained in Section \ref{s:lookup:small}, we use a single helper function for the entire fractional expression in \eqref{e:lookup:fractional:identity}, which is subject to a domain identity over $H$ which has $\bigO{M}$ variables and absolute degree $\bigO{M}$.
...
This yields a sumcheck polynomial in $\bigO{M}$ many variables, but the absolute degree is bounded, henceforth having a $\bigO M$ prover.
...
Correctness of $h$ is enforced by the domain identity
\begin{equation}
...
By Remark \ref{rem:Lagrange}, the values for $L_H(\vec X, \vec y)$ over $H$ are obtained within $|H|\cdot (\mathsf M + \mathsf A)$ operations. 
...
For this we use a heuristic for the cost of computing commitments by a multi-scalar multiplication in an elliptic curve over $256$ bit large field.
According to \cite{ECscalarMul} a lower bound for the cost of a single scalar multiplication is about $8\cdot 256 = 2040$ field multiplications. 
For the multi-scalar multiplication of size $|H|$ using the Pippenger algorithm, we take the cost of $|H|$ scalar multiplications corrected by a factor of $\frac{1}{\log|H|}$, which yields about 
\[
\frac{|H|}{\log|H|}\cdot 2040 \cdot\mathsf M 
\]
field multiplications for a single commitment.
Using this estimate the number of multiplications for the $\bigO{M}$-strategy is 
\begin{equation}
\label{lookup:cost:Mcolumn:naive}
|H|\cdot (43\cdot M + \frac{2040}{\log|H|}\cdot (M+1)) \cdot\mathsf M,
\end{equation}
wheras for the multi-column lookup it is
\begin{equation}
\label{lookup:cost:Mcolumn:batch}
|H| \cdot  \big((5\cdot M^2 + 19 \cdot M + 18 + \frac{4080}{\log|H|}) \cdot\textsf M.
\end{equation}
Comparing \eqref{lookup:cost:Mcolumn:naive} and \eqref{lookup:cost:Mcolumn:batch} at $|H|=2^{12}$ yields an expected maximum speedup by a factor of $2.15$ for $M=8$, more than $50\%$ faster for $3\leq M\leq 21$, and a break-even point at about $M = 36$ columns.
...
In short, the argument is based on the fact that, given two sequences of field elements $(a_i)_{i=0}^{N-1}$ and $(t_i)_{i=0}^{N-1}$, then $\{a_i : j= 0,\ldots, N-1\}\subseteq \{t_i : i = 0, \ldots, N-1\}$ as sets, if and only if there exists a sequence $(s_i)_{i=0}^{2N-1}$ of double the size, which satisfies the identity
\begin{equation}
...
\subsection{Batched grand product for not too many columns}
...
The prover provides the ordered merge of the $f_i$  together with $t$ via additional functions $s_i: H'\rightarrow F$, $i=1,\ldots, M+1$, subject to the Gabizon identity
\begin{multline*}
...
= 0, 
\end{equation}
for all $\vec x \in H$, where for $i=1,\ldots, M$,
\begin{align*}
...
Notice that $Q$ has  absolute degree $d = M+3$, which is the same as in Protocol \ref{prot:lookup}, but  about the double of variables, $\nu= 2\cdot M+ 6$.
...
Using the same heuristics for the cost of a multi-scalar multiplication as in Section \ref{s:lookup:cost}, again for $n=12$ we expect Protocol \ref{prot:lookup} more than $75\%$ faster at almost the entire range $3\leq M \leq 28$, achieving a maximum speedup of about $130\%$ at $M=9$ columns.

\subsection{Large number of columns}
...
This leads to the following prover cost.
...
\] 
Neglecting the $\frac{1}{|H|}$-term, the overall cost of the prover is
\begin{equation}
...
%As the grand product extends over the entire hypercube $\bar H$, the grand product argument from Setty and Lee \cite{Quarks} is m



\subsection{Comparison with logarithmic derivative lookups (TODO)}
...
\subsection{IPA for Lagrange-queries (TODO)}

\begin{itemize}
\item
Commitment of Lagrange representation, i.e. the values of a function $f: H\rightarrow F$.
\item
The inner product argument for $\langle f, L_H(\:.\:, \vec z)\rangle$ is as usual, but uses the decomposition into $f(+1, X_2,\ldots, X_n)$ and $f(-1, X_2,\ldots, X_n)$ instead.
Start with $f_0 = f$ over $H_0 = H = \{\pm 1\}^n$, and compute
\[
f_{i+1}(X_{i+1},\ldots, X_n) = \frac{1-\lambda_{i+1}}{2}\cdot f_i(+1, X_{i+1},\ldots, X_n) + \frac{1+\lambda_{i+1}}{2}\cdot f(+1, X_{i+1},\ldots, X_n), 
\] 
where $\lambda_{i}$ is the randomness of the reduction step.
(The correction factors are...)
Lagrange functions $L_i(\pm1\|\vec x, \vec z)$ for $H_i = \{\pm 1\}^{n-i}$ fold into 
\begin{multline*}
\frac{1 + \lambda_{i+1}}{2}\cdot L_{i}(+ 1\|\vec x, \vec z) + \frac{1 - \lambda_{i+1}}{2}\cdot  L_{i}(-1\|\vec x, \vec z) =
\\
= \frac{1 + \lambda_{i+1}}{2} \cdot \frac{1 + z_i}{2}\cdot L_{i+1}(\vec x, \vec z) + \frac{1 - \lambda_{i+1}}{2}\cdot \frac{1-z_i}{2}\cdot L_{i+1}(\vec x, \vec z)
= \frac{1 + \lambda_{i+1}\cdot z_i}{2}\cdot L_{i+1}(\vec x, \vec z).
\end{multline*}

\item
The reduction polynomial is
\[
\prod_{i=1}^{n} \frac{1 + \lambda_i\cdot z_i}{2} = L_n(\vec \lambda, \vec z), 
\]
which is succinct and therefore allows to adapt the Halo public aggregation scheme.
\end{itemize}

\subsection{KZG as Lagrange commitment scheme (TODO)}

\begin{itemize}
\item 
The KZG for multivariate polynomials can be turned into one for Lagrange queries, 
using the commitments of $L_H(\vec X, y)$, $y\in H$, as modified base points. 
By linearity of the scheme, 
\[
\comm(f(\vec X))  =  \comm(\sum_{\vec y\in H} f(\vec y)\cdot L_H(\vec X, \vec y)) = \sum_{\vec y\in H} f(\vec y) \cdot \comm(L_H(\vec X, \vec y)),
\]
without the need to compute the coefficients of $f(X)$. 

\item
The opening proof uses the multivariate ideal representation
\[
f(X_1,\ldots, X_n)  = v + q_1(\vec X)\cdot (X_1 - z_1) + q_n(\vec X) \cdot (X_n - z_n),
\]
which is obtained recursively by starting with $f_1(\vec X) = f(\vec X)$, and computing 
\[
f_i(X_i, X_{i+1}, \ldots, X_n) = q_{i+1}(X_{i+1},\ldots, X_n)\cdot (X_i - z_i) + f_{i+1}(X_{i+1}, \ldots, X_n),
\]
for $i=1, \ldots, n$, via the evaluation reduction for $X_i = z_i$, i.e.
\begin{multline*}
f_i (X_i, \ldots, X_n) = \frac{1 + X_i}{2} \cdot f_i(+1, X_{i+1}, \ldots, X_n) + \frac{1 - X_i}{2} \cdot f_i(-1, X_{i+1}, \ldots, X_n)
\\
=  \frac{1 + X_i - z_i + z_i}{2} \cdot f_i(+1, X_{i+1}, \ldots, X_n) + \frac{1 - X_i + z_i - z_i}{2} \cdot f_i(-1, X_{i+1}, \ldots, X_n).
\end{multline*}
(In the last step $i=n$, $q_{n+1}$ and $f_{n+1}$ are constants. In particular $f_{n+1} = v$.)
which leads to 
\begin{align*}
q_{i+1}(X_{i+1}, \ldots, X_n) &= f_{i, -} (X_{i+1}, \ldots, X_n)
\\
f_{i+1}(X_{i+1},\ldots, X_n) &
%= \frac{1+ z_i}{2} \cdot f_i(+1, X_{i+1}, \ldots, X_n) - \frac{1- z_i}{2}\cdot f_i(-1, X_{i+1}, \ldots, X_n) 
= f_i^-(X_{i+1}, \ldots, X_n) + z_i \cdot f_i^+(X_{i+1}, \ldots, X_n),
\end{align*}
where
\begin{align*}
 f_{i}^+ (X_{i+1}, \ldots, X_n) &= \frac{f_i(+1, X_{i+1}, \ldots, X_n) + f_i(-1, X_{i+1}, \ldots, X_n)}{2}, 
\\
 f_{i}^- (X_{i+1}, \ldots, X_n) &= \frac{f_i(+1, X_{i+1}, \ldots, X_n) - f_i(-1, X_{i+1}, \ldots, X_n)}{2}. 
\end{align*}
From these formulas we see that the values of $f_i^+, f_i^-, q_{i+1}, f_{i+1} : H_{i+1}\rightarrow F$ are obtained from the values of $f_i: H_i\rightarrow F$ within at most $6\cdot |H_{i+1}|$ field operations.
These domain evaluations can be used to determine their commitments in $|H_{i+1}|$ group operations, given the commitments of the Lagrange polynomials $L_{i+1}(\vec X, \vec y)$, $\vec y\in H_{i+1}$. 
They can be obtained from the ones of $L_i(\vec X, \vec y)$, $\vec y \in H_i$ within $|H_{i+1}|$ group operations, using
\begin{multline*}
L_{i+1} (X_{i+1},\ldots, X_n, y_{i+1}, \ldots, y_n)
\\
= L_{i}(X_i ,X_{i+1},\ldots, X_n, +1,  y_{i+1}, \ldots, y_n) + L_{i}(X_i, X_{i+1},\ldots, X_n,  -1,  y_{i+1}, \ldots, y_n).
\end{multline*}
Overall, reduction step $i$ can be performed within $6\cdot |H_{i+1}|$ field operations and $2\cdot |H_{i+1}|$ group operations.
Since $\sum_{i=1}^{n-1} |H_{i+1}| \leq |H|$ the total number of field and group operations are linear in $|H|$.


\item
While the Lagrange KZG scheme is linear-time, the proof size and verifier cost are $O(\log|H|)$.
It seems intriguing to leverage the generic transformation for tensor and polynomial commitment schemes \cite{Gemini, Hyperplonk}  to turn the univariate KZG into a multivariate Lagrange commmitment scheme.
The odd-even decomposition of their univariate representation corresponds exactly to the decomposition of $f_i$ into $f_i^+$ and $f_i^-$, so their commitments can be computed from the values over $H_{i+1}$ without computing the coefficients.
Although the transformation can be performed within $O(|H|)$ field and group operations, the final batched opening proof lacks of a possibility to compute the univariate quotient 
\[
h(x) = \frac{f(x) - v}{ x - z}
\]
in linear time.
