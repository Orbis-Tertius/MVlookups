...
\label{prot:PA}
Assume that $F$ is a finite field with characteristic $p > 2^n$, and let $f,g :H\rightarrow F$ be any pair of functions on the Boolean hypercube $H=\{\pm 1\}^n$.
The tensor IOP for that $f = \{f(\vec x) : \vec x\in H\}\subseteq \{g(\vec x) : \vec x\in H\}$ is as follows.
\begin{enumerate} 
\item
The prover determines the multiplicity function $m:H\rightarrow F$ such that
\[
\sum_{\vec x\in H} \frac{1}{X + f(\vec x)} = \sum_{\vec x\in H} \frac{m(\vec x)}{X + g(\vec x)},  
\]
...
\item
\label{i:PAstep1}
Given a random sample $x$ from the verifier,  the prover determines the logarithmic derivative function $h: H\longrightarrow F$
such that for all $\vec x$ in $H$,
\begin{align} 
\label{e:m}
...
\begin{equation*}
Q(L,h,m, f_x, g_x) =   
L\cdot  \left(h \cdot f_x \cdot g_x + h\cdot f_x-  g_x\right)\cdot  f_x \cdot g_x +  \lambda \cdot h.
\end{equation*}
The sumcheck protocol outputs the expected value $v$ for the multivariate polynomial 
\begin{equation}
\label{e:Qlookup}
\begin{aligned}
Q(L_H(\vec X, \vec y), h(\vec X), m(\vec X), x + f(\vec X),  z_2 + g(\vec X))
\end{aligned}
\end{equation}
at $\vec X=\vec r$ sampled by the verifier in the course of the protocol.
...
to check whether \eqref{e:Qlookup} equals the expected value $v$ at $\vec X = \vec r$. 
(The value $L_H(\vec r, \vec y)$ is computed by the verifier.)
\end{enumerate}
\end{protocol}
